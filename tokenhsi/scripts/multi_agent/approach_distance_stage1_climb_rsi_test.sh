#!/bin/bash
# <checkpoint> [num_agents=2] [num_envs=1] [num_objects=3] [eval_repeats=10]
...
NUM_OBJECTS=${4:-3}
NUM_REPEATS=${5:-10}
RELATION_ENV_CFG=tokenhsi/data/cfg/multi_agent/approach_distance_stage1_climb_rsi.yaml
RELATION_OUTPUT_DEFAULT=output/approach_distance_stage1_climb_rsi
set -- --task_graph "${TASK_GRAPH:-climb}" --task_camera "${TASK_CAMERA:-stack}"
...
if [ "${HEADLESS:-0}" != 0 ]; then set -- "$@" --headless; fi

...
NUM_OBJECTS=${4:-3}
NUM_REPEATS=${5:-3}
RELATION_ENV_CFG=tokenhsi/data/cfg/multi_agent/approach_distance_edge_context_ontop.yaml
RELATION_OUTPUT_DEFAULT=output/approach_distance_edge_context_ontop
set -- --task_graph "${TASK_GRAPH:-at_ontop}" --task_camera "${TASK_CAMERA:-stack}"
if [ "${TASK_ROLE_SWAP:-0}" = 1 ]; then
    set -- "$@" --task_role_swap
fi
if [ -n "${RELATION_GRAPH:-}" ]; then
    set -- "$@" --relation_graph "$RELATION_GRAPH"
fi
if [ -n "${EPISODE_LENGTH:-}" ]; then
    set -- "$@" --episode_length "$EPISODE_LENGTH"
fi
if [ -n "${SEED:-}" ]; then
    set -- "$@" --seed "$SEED"
fi

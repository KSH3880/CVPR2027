#!/bin/bash
# [num_agents=2] [num_envs=2048] [num_objects=3]; Stage-1 primitive-RSI scratch training.
. "$(dirname "$0")/runtime_env.sh"
set -eu
NUM_AGENTS=${1:-2}
NUM_ENVS=${2:-2048}
NUM_OBJECTS=${3:-3}
RELATION_ENV_CFG=tokenhsi/data/cfg/multi_agent/approach_distance_edge_context_stage1_primitives_rsi.yaml
RELATION_OUTPUT_DEFAULT=output/approach_distance_edge_context_stage1_primitives_rsi
...
python ./tokenhsi/run.py --task HumanoidMACarry \
    ...
    --experiment ApproachDistanceEdgeContextStage1PrimitivesRsi \
    --headless --no_video "$@"

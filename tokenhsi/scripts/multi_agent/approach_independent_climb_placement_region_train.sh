...
python ./tokenhsi/run.py --task HumanoidMACarry \
  ...
  --cfg_env tokenhsi/data/cfg/multi_agent/approach_independent_climb_placement_region.yaml \
  --motion_file tokenhsi/data/dataset_loco_sit_carry_climb.yaml \
  --num_envs "$NUM_ENVS" --num_agents "$NUM_AGENTS" --num_objects "$NUM_OBJECTS" \
  --output_path "${OUTPUT_PATH:-output/approach_independent_climb_placement_region}" \
  --experiment ApproachIndependentClimbPlacementRegion --headless --no_video "$@"

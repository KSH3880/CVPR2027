...
NUM_OBJECTS=${4:-3}
NUM_REPEATS=${5:-3}
set -- --task_graph "${TASK_GRAPH:-holding_at}" --task_camera "${TASK_CAMERA:-stack}"
...
python ./tokenhsi/run.py --task HumanoidMACarry \
  ...
  --cfg_env tokenhsi/data/cfg/multi_agent/approach_scenario_no_climb.yaml \
  --motion_file tokenhsi/data/dataset_loco_sit_carry_no_climb.yaml --checkpoint "$CKPT" \
  ...
  --output_path "${OUTPUT_PATH:-output/approach_scenario_no_climb}" \
  --no_video --test --eval "$@"

#!/bin/bash
# Sampled OnTop experiment: one-command remote viewer.
set -eu
...
export OUTPUT_PATH=${OUTPUT_PATH:-output/approach_distance_edge_context_ontop_vnc}
...
exec bash "$SCRIPT_DIR/run-gui.sh" \
    bash "$SCRIPT_DIR/approach_distance_edge_context_ontop_test.sh" \
    "$1" "${2:-2}" "${3:-1}" "${4:-3}" "${5:-10}"

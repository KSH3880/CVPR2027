...
NUM_OBJECTS=${4:-3}
NUM_REPEATS=${5:-3}
RELATION_ENV_CFG=tokenhsi/data/cfg/multi_agent/approach_distance_edge_context_interaction.yaml
RELATION_OUTPUT_DEFAULT=output/approach_distance_edge_context_interaction
# Fixed by default so a one-env viewer is interpretable; TASK_GRAPH=random remains available.
set -- --task_graph "${TASK_GRAPH:-hold_sit}" --task_camera "${TASK_CAMERA:-stack}"

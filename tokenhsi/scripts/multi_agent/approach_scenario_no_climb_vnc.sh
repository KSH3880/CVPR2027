...
export OUTPUT_PATH=${OUTPUT_PATH:-output/approach_scenario_no_climb_vnc}
export TASK_GRAPH=${TASK_GRAPH:-holding_at}
exec bash "$SCRIPT_DIR/run-gui.sh" \
  bash "$SCRIPT_DIR/approach_scenario_no_climb_test.sh" \
  "$1" "${2:-2}" "${3:-1}" "${4:-3}" "${5:-10}"

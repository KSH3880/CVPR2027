#!/bin/bash
# [num_agents=2] [num_envs=2048] [num_objects=3]; schema-5 Stage-1 ground-target scratch training.
. "$(dirname "$0")/runtime_env.sh"
set -eu
NUM_AGENTS=${1:-2}
NUM_ENVS=${2:-2048}
NUM_OBJECTS=${3:-3}
RELATION_ENV_CFG=tokenhsi/data/cfg/multi_agent/approach_distance_edge_context_stage1_ground_sit_climb.yaml
RELATION_OUTPUT_DEFAULT=output/approach_distance_edge_context_stage1_ground_sit_climb
...
python ./tokenhsi/run.py --task HumanoidMACarry \
    ...
    --experiment ApproachDistanceEdgeContextStage1GroundSitClimb \
    --headless --no_video "$@"

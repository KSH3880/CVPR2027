#!/bin/bash
# lab 슬롯 한 칸. steer 와 ma 양쪽을 같은 방식으로 잰다.
#
...
printf 'LAB tag=%s repo=%s envs=%s cp=%sM spacing=%s cell=%s rc=%s warn=%s needM=%s sfps=%s\n' \
    "$TAG" "$REPO" "$ENVS" "$CP" "${LAB_SPACING:-orig}" "${LAB_CELL:-60}" "$rc" \
    "${warn:-0}" "$(awk -v n="${need:-0}" 'BEGIN{printf "%.1f", n/1048576}')" "${sfps:-NA}"
exit $rc

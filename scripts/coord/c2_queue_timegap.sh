#!/bin/bash
# Start the direct crossing-time-gap experiment after the peak trainer exits.
set -uo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
WAIT_TAG=${1:-c2_k1_peakcol_s0}
TAG=${2:-c2_k1_timegap_s0}
...
bash "$ROOT/scripts/coord/c2_timegap_compare.sh" "$TAG" "$EXECUTOR" &
runner_pid=$!

#!/bin/bash
# Joint (x,y,v) MLP with compact state-derived crossing features.
set -eo pipefail
...
if [ -f "$ROOT/runs/coord/$TAG/coord_c2_000500.pth" ]; then
    echo "C2_CONFLICTFEAT_REUSE_COMPLETE tag=$TAG"
    exit 0
fi
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
    echo "C2_CONFLICTFEAT_REUSE_RUNNING tag=$TAG"
...
MS_SCEN=cross COORD_MODEL=c2 COORD_C2_MLP_BACKBONE=1 \
    COORD_C2_MLP_CONFLICT_FEATURES=1 \

#!/bin/bash
# End-to-end joint (x,y,v) with frozen-ms18 measured timing in future loss.
set -eo pipefail
...
if [ -f "$ROOT/runs/coord/$TAG/coord_c2_000500.pth" ]; then
    echo "C2_FUTURE8_MEASTIME_REUSE_COMPLETE tag=$TAG"
    exit 0
fi
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
    echo "C2_FUTURE8_MEASTIME_REUSE_RUNNING tag=$TAG"

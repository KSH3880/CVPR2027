#!/bin/bash
# Launch strong-yield training and evaluate only numbered milestones in parallel.
set -uo pipefail
...
bash "$ROOT/scripts/coord/c2_jointpoint_yield0375_compare.sh" \
    "$TAG" "$EXECUTOR" &
runner_pid=$!
...
echo "C2_YIELD0375_DONE tag=$TAG runner_status=$runner_status watcher_status=$watcher_status"

#!/bin/bash
# Start fixed-priority training after the h15 trainer frees a slot.
set -uo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
WAIT_TAG=${1:-c2_k1_immediate_h15_s0}
TAG=${2:-c2_k1_priority_s0}
...
bash "$ROOT/scripts/coord/c2_priority_compare.sh" "$TAG" "$EXECUTOR" &
runner_pid=$!

#!/bin/bash
# Robust-time end-to-end joint (x,y,v), with collision loss reduced 50 -> 10.
set -eo pipefail
...
if [ -f "$ROOT/runs/coord/$TAG/coord_c2_000500.pth" ]; then
    echo "C2_FUTURE8_T075_C10_REUSE_COMPLETE tag=$TAG"
    exit 0
fi
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
    echo "C2_FUTURE8_T075_C10_REUSE_RUNNING tag=$TAG"
...
MS_SCEN=cross COORD_MODEL=c2 COORD_C2_MLP_BACKBONE=1 \
    ...
    COORD_C2_FUTURE_COLLISION_COEF=10 \

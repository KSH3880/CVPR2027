#!/bin/bash
# Launch the conflict-feature comparison and evaluate numbered milestones.
set -uo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
TAG=${1:-c2_k1_jointpoint_conflictfeat_y0375_s0}
EXECUTOR=${2:-ms18_maskteam_origscale_c06_s0}

bash "$ROOT/scripts/coord/c2_jointpoint_conflictfeat_compare.sh" \
    "$TAG" "$EXECUTOR" &
runner_pid=$!
...
echo "C2_CONFLICTFEAT_DONE tag=$TAG runner_status=$runner_status watcher_status=$watcher_status"

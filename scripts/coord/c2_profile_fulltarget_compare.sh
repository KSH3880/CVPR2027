#!/bin/bash
# C2 K=1: state-ordered current-to-cross slowdown plus explicit max-speed restore.
set -eo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
TAG=${1:-c2_k1_profile_fulltarget_s0}
EXECUTOR=${2:-ms18_maskteam_origscale_c06_s0}

MS_SCEN=cross COORD_MODEL=c2 COORD_C2_DIRECT_SPEED_PROFILE=1 \
    COORD_C2_STATE_ORDERED_TARGET_GAP=1 COORD_C2_ARRIVAL_TIME_MARGIN=1.0 \
    COORD_C2_ARRIVAL_GAP_COEF=10.0 \

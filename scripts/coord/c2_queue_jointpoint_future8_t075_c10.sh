#!/bin/bash
# Launch lower-weight robust-time training and milestone evaluation.
set -uo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
TAG=${1:-c2_k1_jointpoint_future8_t075_c10_s0}
EXECUTOR=${2:-ms18_maskteam_origscale_c06_s0}

bash "$ROOT/scripts/coord/c2_jointpoint_future8_t075_c10_compare.sh" \
    "$TAG" "$EXECUTOR" &
runner_pid=$!
...
echo "C2_FUTURE8_T075_C10_DONE tag=$TAG runner_status=$runner_status watcher_status=$watcher_status"

#!/bin/bash
# Start after the whole prefix-target runner exits, including final eval.
set -uo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
WAIT_TAG=${1:-c2_k1_profile_ordered_s0}
TAG=${2:-c2_k1_profile_features_s0}
EXECUTOR=${3:-ms18_maskteam_origscale_c06_s0}

echo "C2_QUEUE_WAIT_RUNNER wait_tag=$WAIT_TAG next_tag=$TAG"
while pgrep -f "[c]2_profile_ordered_compare.sh $WAIT_TAG" >/dev/null; do sleep 20; done
echo "C2_QUEUE_START tag=$TAG"
bash "$ROOT/scripts/coord/c2_profile_features_compare.sh" "$TAG" "$EXECUTOR" &
runner_pid=$!

#!/bin/bash
# Train random-priority consistency C2 and evaluate milestones.
set -uo pipefail
...
bash "$ROOT/scripts/coord/c2_jointpoint_future8_meastime_cons1_compare.sh" \
    "$TAG" "$EXECUTOR" &
runner_pid=$!
...
echo "C2_FUTURE8_MEASTIME_CONS1_DONE tag=$TAG runner_status=$runner_status watcher_status=$watcher_status"

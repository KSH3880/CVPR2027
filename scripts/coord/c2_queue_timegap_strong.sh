#!/bin/bash
# Start strong time-gap training after the immediate trainer frees a slot.
set -uo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
WAIT_TAG=${1:-c2_k1_immediate_s0}
TAG=${2:-c2_k1_timegap20_s0}
...
bash "$ROOT/scripts/coord/c2_timegap_strong_compare.sh" "$TAG" "$EXECUTOR" &
runner_pid=$!

#!/bin/bash
# Preserve live experiments; start the state-ordered profile when timegap20 exits.
set -uo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
WAIT_TAG=${1:-c2_k1_timegap20_s0}
TAG=${2:-c2_k1_profile_ordered_s0}
...
bash "$ROOT/scripts/coord/c2_profile_ordered_compare.sh" "$TAG" "$EXECUTOR" &
runner_pid=$!

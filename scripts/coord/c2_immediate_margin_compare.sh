#!/bin/bash
# C2 immediate slowdown with one larger predictive human-clearance margin.
set -eo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
TAG=${1:-c2_k1_immediate_h15_s0}
EXECUTOR=${2:-ms18_maskteam_origscale_c06_s0}

MS_SCEN=cross COORD_MODEL=c2 COORD_C2_IMMEDIATE_SLOWDOWN=1 \
    COORD_C2_HUMAN_CLEARANCE=1.5 COORD_ITERS=500 COORD_SAVE_EVERY=10 COORD_ENVS=64 \

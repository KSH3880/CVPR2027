#!/bin/bash
# Launch robust-time end-to-end training and milestone evaluation.
set -uo pipefail
...
bash "$ROOT/scripts/coord/c2_jointpoint_future8_t075_compare.sh" \
    "$TAG" "$EXECUTOR" &
runner_pid=$!
...
echo "C2_FUTURE8_T075_DONE tag=$TAG runner_status=$runner_status watcher_status=$watcher_status"

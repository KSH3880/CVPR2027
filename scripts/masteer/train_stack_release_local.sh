#!/bin/bash
# Continue the shared ms18 policy on sequential two-box stacking.
set -eo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
TAG=${1:-ms21_stackrel_pathclear_s0}

export TOKENHSI_CONDA_ENV=${TOKENHSI_CONDA_ENV:-tokenhsi_koo}
if [ "${MS_DDP_N:-1}" -gt 1 ]; then
    # torchrun이 CUDA_VISIBLE_DEVICES에 보이는 GPU를 rank별로 할당한다.
    unset MA_GPU
else
...
# Reward-only release and centralized dependency controller.

...
# 관측/attention 설정은 학습과 같아야 한다. 명령행에서 MA_TOKEN을 직접 줬으면
# 그 값을 우선하고, 아니면 태그 sidecar의 값만 복원한다. 전체 sidecar를 source하면
# MS_VIZ용 사용자의 시나리오 override까지 덮으므로 여기서는 이 축만 읽는다.
if [ -z "${MA_TOKEN+x}" ] && [ -n "$SOURCE_TAG" ]; then
    ENV_FILE="$ROOT/runs/queue/logs/$SOURCE_TAG.env"
    if [ -f "$ENV_FILE" ]; then
        MA_TOKEN=$(bash -c 'source "$1"; printf "%s" "${MA_TOKEN:-live}"' _ "$ENV_FILE")
        export MA_TOKEN
    fi
fi
...
echo " env        $ENVS x 2명  배치=$MS_SCEN  CLIP=$MS_CLIP  카메라=$MS_CAM"

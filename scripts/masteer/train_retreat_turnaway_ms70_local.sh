#!/bin/bash
# ms70: keep a linear yaw-progress signal throughout the turn, then increase
# its cosine-shaped gain inside 90 degrees before walking and stopping.
set -euo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
TAG=${1:-ms70_ms18e9000_hybridyaw15_cos2_at90_wait10_hold10_retreat2to3_3000_s0}
GPU=${MA_GPU:-7}
if [[ "$GPU" != 6 && "$GPU" != 7 ]]; then
...
# A single CLEAR phase: always reward reduced yaw error; once the remaining
# error is at most 90 degrees, increase its gain toward the target with cosine.
...
export PILOT_TOP_WAIT_REWARD_W=10.0
export PILOT_BASE_HOLD_REWARD_W=10.0

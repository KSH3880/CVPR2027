#!/bin/bash
# Headless coordinator + sequential-stack evaluation (separate from baseline).
# COORD_CKPT=/abs/coord.pth bash scripts/masteer/eval_coord_sequential_stack.sh /abs/masteer.pth [tag]
#
# Usage:
#   MA_GPU=7 STACK_EVAL_ENVS=270 \
#     bash scripts/masteer/eval_coord_sequential_stack.sh /abs/Humanoid.pth [tag]
# Default grid sizes are testSizes indices 0,4,7 = 0.22/0.42/0.57 m cubes.
# Override with STACK_EVAL_BOX_SIZE_IDS=i,j,k.
#
# Results:
#   runs/results/coord_sequential_stack/eval_<tag>.npy
#   runs/results/coord_sequential_stack/eval_<tag>.log
set -eo pipefail

ROOT=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)
REPO="$ROOT/TokenHSI-masteer"
if [ "${1:-}" = --help ]; then
    echo "COORD_CKPT=/abs/coord.pth MA_GPU=0 bash $0 /abs/masteer.pth [tag]"
    echo "Results: runs/results/coord_sequential_stack/eval_<tag>.{npy,log}"
    exit 0
fi
POLICY=${1:?usage: eval_coord_sequential_stack.sh <policy.pth> [tag]}
TAG=${2:-$(basename "$POLICY" .pth)}
ENVS=${STACK_EVAL_ENVS:-270}
GPU=${MA_GPU:-0}
STAGE1=${MS_CKPT:-"$REPO/output/ckpt_stage1.pth"}
RESULT_DIR="$ROOT/runs/results/coord_sequential_stack"
...
CFG=$(mktemp /tmp/coord_stack_eval.XXXXXX.yaml)
cleanup() { rm -f -- "$CFG"; }
...
echo "planner=$COORD_CKPT replan=$COORD_REPLAN_STEPS accel=$COORD_COMMAND_ACCEL" | tee -a "$LOG"
cd "$REPO"
...
"$TOKENHSI_PYTHON" - "$TAG" "$METRICS" "$ENVS" "$BOX_GRID" <<'PY' | tee -a "$LOG"
import sys
import numpy as np

# TokenHSI — 이 머신에서의 설치 구성 및 사용법

이 문서는 이 서버(RTX 4080 / CUDA 11.7 / Ubuntu 22.04)에 실제로 구축·검증된 환경 기준입니다.
2026-08-18 구축, 추론 13종 + 학습 2종 실행 검증 완료.
...
| PyTorch | 2.0.0+cu118 |
| IsaacGym | Preview 4 (`~/Downloads/isaacgym`, editable 설치) |
| pytorch3d | 0.7.7 (소스 빌드) |
| 저장소 | `~/JHH/CVLAB/Projects/TokenHSI` |
| 데이터 | `tokenhsi/data/` (약 2GB) |
| 체크포인트 | `output/` (13개, 약 1GB) |

### 시스템 환경을 건드리지 않은 부분

- 시스템 CUDA는 **11.7 그대로**입니다. PyTorch 휠은 자체 CUDA 런타임을 번들로 포함하므로
  시스템 툴킷과 무관하게 동작합니다. 실제 제약은 드라이버(535, CUDA 12.2 지원)입니다.
- pytorch3d 컴파일용 CUDA 11.8 nvcc는 **conda 환경 내부에만** 설치되어 있습니다.
- `LD_LIBRARY_PATH`는 `~/.bashrc`가 아니라 conda activate 훅
  (`$CONDA_PREFIX/etc/conda/activate.d/tokenhsi_env.sh`)에서 설정됩니다.
  → 다른 conda 환경(detr3d, maft 등)에 영향 없음.
- 원격 GUI 스택(TurboVNC/VirtualGL/noVNC)은 **root 없이** `~/.local`, `~/opt`에 설치되어 있습니다.

### 시작하기

```bash
conda activate tokenhsi
cd ~/JHH/CVLAB/Projects/TokenHSI
```
...
```bash
~/opt/vnc/start-vnc.sh                                        # 최초 1회
~/opt/vnc/run-gui.sh tokenhsi/scripts/single_task/traj_test.sh
```
...
MacBook → VS Code Remote-SSH 환경에서 IsaacGym 창을 브라우저로 보는 구성입니다.
**root 권한 없이** 설치되어 있습니다.
...
```bash
~/opt/vnc/start-vnc.sh              # 기본 :2, 1920x1080
~/opt/vnc/start-vnc.sh 2 1600x1000  # 해상도 지정
```
...
```bash
~/opt/vnc/run-gui.sh tokenhsi/scripts/single_task/traj_test.sh
~/opt/vnc/run-gui.sh tokenhsi/scripts/tokenhsi/stage1_test.sh
```
...
- **IsaacGym은 OpenGL이 아니라 Vulkan으로 렌더링합니다.** 따라서 VirtualGL은
  ...
  (이 머신에는 ICD가 6개 — intel, llvmpipe, radeon 등 — 있어 지정하지 않으면
  소프트웨어 드라이버가 선택될 수 있습니다.)
- 검증: `nvidia-smi`에서 프로세스가 `C+G`(Compute+Graphics)로 표시되면 GPU 렌더링입니다.
- 순수 OpenGL 앱은 `USE_VGL=1`로 VirtualGL을 켜세요:
  ```bash
  USE_VGL=1 ~/opt/vnc/run-gui.sh glxgears
  ```
...
```bash
~/.local/opt/TurboVNC/bin/vncpasswd
...
| `No module named 'pytorch3d'` | `conda activate tokenhsi` 누락. pytorch3d는 **모든** 태스크의 필수 의존성입니다 (`tokenhsi/utils/parse_task.py`에서 무조건 import). |
...
| `libvglfaker.so cannot be preloaded` | `LD_LIBRARY_PATH=~/.local/opt/VirtualGL/lib` 필요. `run-gui.sh`는 자동 처리. |
...
> 현재 SMPL 모델은 설치되어 있지 않습니다. 학습·추론에는 불필요합니다.

...
- 모든 탐색은 요약에서 시작하고 필요한 범위만 확장한다. 사용자가 전체 출력을 요구하거나
  전체 일관성 검토가 필요한 경우가 아니면 파일·로그·명령 결과를 통째로 출력하지 않는다.
- 셸 출력은 명령 단계에서 `rg -m`, `head`, `tail`, 제한된 `sed` 범위, 집계용 `awk` 등으로
  제한한다. 도구의 사후 출력 잘림에 의존하지 않는다.
- `git status`와 `git status --short`를 무제한으로 실행하지 않는다. 먼저 porcelain 출력을
  상태별 개수로 집계하고, 실제 경로는 작업 대상 디렉터리로 좁히거나 일부만 표시한다.
- diff 탐색은 `git diff --stat` 또는 `--numstat`로 시작하고, 이후 작업 파일의 diff만 읽는다.
  저장소 전체 diff나 파일 목록을 한 번에 출력하지 않는다.
- 로그는 관련 패턴 검색과 최신 100줄 이내 확인으로 시작한다. 충분하지 않을 때만 범위를
  단계적으로 늘리고, 반복되는 진행 로그는 매번 다시 출력하지 않는다.
- 50KB를 넘는 Markdown과 결과 원장은 먼저 크기와 제목 목록을 확인한다. 태그·날짜·메트릭을
  `rg`로 찾아 관련 섹션만 읽으며, `AUTO_RESULTS`와 `EXPERIMENTS` 전체를 기본 컨텍스트로
  적재하지 않는다. 이 문서의 "읽는다"는 필요한 부분을 조회한다는 뜻이다.
- 한 명령의 예상 출력이 불명확하면 먼저 개수만 세거나 작은 표본을 확인한다. 예상 밖의 큰
  출력이 발생하면 같은 출력을 재호출하지 말고 이미 확인한 요약을 기준으로 다음 범위를 좁힌다.

...
## 2026-08-31 · Stage1 고정 joint carry/steering/STOP

- A checkpoint 전체를 고정하는 실험과 별도로 `MSTOP_POLICY_MODE=joint_stop_head`, `MSTOP_INIT_MODE=stage1`을 추가했다. Stage1 self/기존 task/Transformer/composer는 고정하고 새 steering/carry/STOP tokenizer, 공용 adapter와 STOP composer/adapter를 함께 학습한다.
- A 6000 checkpoint나 optimizer를 resume하지 않고 Stage1 checkpoint에서 새 optimizer로 시작한다. 전용 런처는 A/free no-stop 30%, B-layout no-stop 30%, B-layout STOP 40% 기대 혼합을 사용한다.
- `MSTOP_STOP_PROB`를 per-env opt-in으로 추가했다. 기본값 1은 기존 B 동작을 보존하며 joint 런처의 0.5714286은 B-layout 중 약 4/7에만 STOP을 활성화한다.
- GPU 6의 16-env 1-iteration smoke가 완주하고 checkpoint를 저장했다. Stage1 core/old-carry는 동결됐고 새 steering/carry tokenizer와 공용 adapter의 nonzero gradient를 확인했다. 짧은 rollout은 STOP 시작 전이라 STOP branch gradient 검증은 기존 5-iteration smoke 결과를 따른다.
- 본 학습 `mastop_joint_stophead_from_stage1_s0`을 GPU 6, 2048 env, 3000 iteration의 독립 user service로 시작했다. 첫 optimizer step에서 모든 새 tokenizer/adapter/head는 nonzero gradient, Stage1 core는 zero gradient임을 확인했다.

## 2026-08-31 · frozen A + STOP tokenizer/head gated mixture

- 기존 residual 모드는 보존하고 `MSTOP_POLICY_MODE=stop_head` opt-in을 추가했다. A branch는 STOP 토큰을 항상 mask한 frozen Transformer/composer/adapter를 사용하고, STOP branch만 21-D command tokenizer와 A에서 복제한 composer/adapter를 학습한다.
- 최종 action은 `A + gate * (STOP - A)`이며 gate 0에서는 clamp도 건너뛰어 A action을 그대로 반환한다. A checkpoint warm-start는 model/RMS/epoch만 복원하고 새 optimizer를 쓰며 STOP head/adapter는 로드된 A branch에서 복제한다.
- brake/hold/resume 전체에 opt-in 양손 contact 보상과 strict grasp 이탈 penalty를 추가했다. 전용 런처는 phase reward와 strict/contact/post-drop/slip/lat/yaw 가중치를 함께 기록한다.
- GPU 6의 16-env 1/5-iteration smoke가 완주했다. 5-iteration 후 frozen composer/Transformer max diff는 0이고 STOP tokenizer/head/adapter만 `4.7e-4~5.8e-4` 변했다. GO 적용량은 0이며 brake/hold/resume rollout을 확인했다.
- 본 학습 `mastop_b_stophead_from_a6k_s0_r1`을 GPU 6, 2048 env, A 6000 checkpoint, 3000 additional iterations로 시작했다. 최초 무접미 태그는 systemd의 빈 `LD_LIBRARY_PATH` 문제로 학습 전 종료됐으며 로그와 sidecar를 보존했다.

## 2026-08-31 · A/B wrapper actor parity trace

- A runner와 같은 JSONL actor trace를 player에 추가하고, parity가 켜졌을 때 기존 residual 진단의 base action, delta, gate, final action도 함께 노출한다.
- 'MSTOP_PARITY_PATH'가 없으면 기존 residual 진단·action 경로와 viewer 동작은 변하지 않는다.
- 같은 exact B PTH가 A runner formal eval에서는 5/6, StopCarry scenario A에서는 0/6 terminate한 원인을 observation과 action의 최초 divergence로 분리한다.

## 2026-08-31 · A/free 배치 위 B 정지 진단 분리

- `MSTOP_A_LAYOUT=1`은 scenario ID를 B로 유지하면서 B의 고정 12m 배치와 직선 path override만 건너뛴다. B의 pickup 후 brake/hold/resume 스케줄과 residual은 그대로 작동하고 위치·목표·경로는 A/free 분포를 사용한다.
- `MS_FLOOR_START=1`을 mastop carry에서도 지원해 `loco_carry` 초기화만 선택한다. `MSTOP_CARRYWITH=1`과 동시에 사용하면 모순이므로 즉시 거부한다.
- 두 옵션의 기본값은 0이라 기존 A/B/C/D 학습·평가 배치와 초기화는 변경하지 않는다.

## 2026-08-30 · stop 실패축 병렬 ablation

- HOLD quality pilot에 양손 OBB worst-hand 보상과 접촉 발 slip, path-frame 횡속도, yaw-rate penalty를 추가했다. 네 가중치의 기본값은 모두 0이라 기존 B reward 경로는 유지된다.
- residual에 `MSTOP_ARM_SCALE`과 `MSTOP_RESUME_RESIDUAL_SCALE`을 추가했다. 기본값은 1이며, arm mask는 action 6:14의 양 어깨·팔꿈치에만 적용한다.
- residual launcher가 새 knob를 sidecar에 보존한다. 8-env smoke와 HOLD/RESUME를 포함한 4-iteration smoke가 rc=0으로 끝났고, HOLD/RESUME count 191/148 및 nonzero residual, NaN·clamp 이상 없음을 확인했다.
- 기존 exact A 6k에서 `mastop_b_holdq_from_a6k_s0_r1`(GPU 6), `mastop_b_armmask_from_a6k_s0_r1`(GPU 6), `mastop_b_resumehalf_from_a6k_s0_r1`(GPU 7)을 각각 2048 env·3000 additional iterations로 시작했다. 각 런은 HOLD reward, arm scale 0, RESUME residual scale 0.5 중 한 축만 바꾼다.

## 2026-08-30 · carry viewer 정합성과 시각 실패 지표

- `scripts/mastop/view.sh`를 object-free `HumanoidMAStopSteer`에서 실제 B 학습 task인 `HumanoidMAStopCarry`로 바꾸고, `output/mastop_carry` config/checkpoint와 학습 sidecar를 복원한다. floor pickup·`MA_SEP=9`·3-repeat eval이 기본이며 continuous viewer는 `MSTOP_VIEW_EVAL=0`으로 선택한다.
- viewer checkpoint snapshot을 `runs/view_snapshots`에 두어 eval metrics가 `/metrics`로 빠지는 권한 오류를 막고, Vulkan UUID guard와 x11vnc `-noshm`을 유지했다.
- 기존 0–60 metric 열은 보존하고 양손 OBB 유지, 손 net-force 보강치, terminal/재파지/post-stop drop, 접촉 발 slip, path-frame 횡속도와 yaw-rate를 61–79열에 append했다. net force는 접촉 상대를 구분하지 못하므로 성공 판정이 아니라 별도 보강치로만 출력한다.
- 8-env floor smoke에서 두 정책 모두 `(48, 80)` 저장과 `MSTOP_STRICT_SUMMARY` 출력을 확인했다. terminal 양손 유지는 BASE/PHASE 모두 0.381, post-stop drop은 0.667/0.619로 기존 frame-average held 0.840/0.949가 시각 실패를 가리는 것을 확인했다.
- 마지막 200 training iter의 기존 phase 진단에서 GO drop은 BASE/PHASE 0.195/0.197(residual 적용 0), HOLD root/box 속도는 0.215/0.296과 0.239/0.289, RESUME drop은 0.031/0.052였다. 기반 집기, HOLD 미끄럼, PHASE resume 과보정을 분리해 해석한다.

## 2026-08-29 · B DDP smoke에서 진단 task reward 원천 수정

- GPU 6·7 2-rank smoke에서 `AMPAgent.play_steps()`가 반환하는 transformed
  batch에 `rewards` key가 없어 `MSTOP_DIAG=1` B가 첫 epoch에서 종료되는
  것을 확인했다.
- task reward 진단은 rollout 동안 shaped task reward를 보관하는
  `experience_buffer.tensor_dict['rewards']`를 동일한
  `swap_and_flatten01` 순서로 읽도록 수정했다. AMP reward와 학습 reward
  결합식은 변경하지 않았다.

## 2026-08-28 · B phase-gated AMP reward ablation

- `MSTOP_PHASE_REWARD`를 추가했다(기본값 `0`). 꺼지면 기존 `AMPAgent._combine_rewards()`를 그대로 호출해 baseline의 `0.5 task + 0.5 AMP` 연산 경로를 보존한다.
- 켜지면 GO=`0.5/0.5`, BRAKE·HOLD=`1/0`, RESUME는 기존 cosine speed scale을 사용해 `task 1→0.5`, `AMP 0→0.5`로 per-env·per-step 결합한다. discriminator freeze/optimizer와 carry/steer 속도 목표는 변경하지 않았다.
- phase별 JSONL에 iteration, target/root/box speed, held/drop, base carry reward, stop bonus, total task/AMP reward, 두 weight, combined reward와 AMP logit을 기록한다.
- 별도 watcher `auto_b_phase_noamp_from_a6k.sh`는 baseline watcher 로그가 남긴 exact 6k checkpoint 경로를 재사용한 뒤 GPU 6·2048 env·3000 iter로 tag `mastop_b_phase_noamp_from_a6k_s0`을 시작한다. claim 내부 PID로 중복 실행을 막는다.
- CPU 단위 검증에서 GO `.5/.5`, BRAKE `1/0`, HOLD `1/0`, RESUME 시작 `1/0`, 중간 `.75/.25`, 종료 `.5/.5`와 flag-off baseline bit-exact 결합을 확인했다. Python/shell compile과 diff check를 통과했다.
- ablation watcher PID `2500846`은 detached 대기 중이다. 점검 시 기존 baseline watcher PID는 없고 baseline claim만 남아 있어, 사용자 지시대로 claim 제거·baseline 재시작은 하지 않았다.

## 2026-08-28 · B 자동 대기 취소와 phase별 원인 진단

- 사용자 요청에 따라 `auto_b_from_a6k.sh` watcher와 claim을 제거했다. 실행 중인 Minimal A GPU 6·7 DDP는 종료하거나 변경하지 않았다.
- 17:12 KST 재요청에 따라 watcher를 다시 시작했다(PID `2343025`, exec session `15635`). A의 정확한 6k checkpoint가 안정화되면 진단이 켜진 B를 GPU 6 단독·총 2048 env로 시작한다.
- B에서만 켜지는 `MSTOP_DIAG=1` 진단을 추가했다. GO/BRAKE/HOLD/RESUME별 root·box·target 속도, 오차, held, stop bonus, task/AMP/combined reward와 discriminator logit을 기록한다.
- frozen A action, residual delta·실제 적용량·포화율, 최종 clamp율과 GO의 final-base 차이도 함께 기록한다. 따라서 명령 미발생, residual 용량/학습 문제, clamp 충돌, drop, AMP 정지 벌점을 분리할 수 있다.
- TensorBoard의 `mstop_diag/<phase>/...`와 `runs/results/mastop_carry/${TAG}_diag.jsonl`에 매 epoch 저장하고, 콘솔은 기본 10 epoch마다 HOLD 요약만 출력한다. 기존 `MS_GRADCHK=1`은 첫 backward에서 residual만 gradient가 생기는지 확인한다.
- motion set에는 전용 carry-hold clip이 없다. `carryWith`의 AMP demo 확률은 0이고, 가장 깨끗한 pickup A5 정지 구간도 전체 positive AMP sampling 기여가 약 3.5%라 HOLD의 phase별 AMP reward 확인이 필요하다.

## 2026-08-28 · A 6k → GPU 6 B 자동 분기

- residual train 실행기가 `MSTOP_GPU=6|7|6,7`을 지원한다. 단일 GPU에서는 총 env 전체를 해당 GPU에 배치하고, 기존 `6,7`은 env를 반씩 나누는 DDP 동작을 유지한다.
- `auto_b_from_a6k.sh`는 실행 중인 A의 정확한 6k checkpoint를 기다리고 파일 안정성·`torch.load`·epoch를 확인한 뒤 B를 GPU 6 단독·총 2048 env·3000 additional iterations로 시작한다.
- 중복 watcher claim과 A 조기 종료 fail-closed를 추가했다. shell syntax, B DDP diff check, 가짜 A PID의 exit 4/claim cleanup을 검증했다.
- watcher PID `2271266`은 17:00 KST에 사용자 요청으로 종료했고 claim도 제거했다. A 프로세스는 변경하거나 종료하지 않았다.


## 2026-08-28 · B residual action 범위 ×2

- B의 stop residual을 `delta=2*tanh(MLP(...))`로 바꿔 관절별 `[-2,2]` 보정이 가능하게 했다. `[-1,1]`인 임의의 A action과 원하는 stop action 사이 차이를 전 범위에서 표현하고, 최종 action은 그대로 `[-1,1]` clamp한다.
- `MSTOP_RESIDUAL_SCALE` 기본값을 2로 추가하고 B 학습 sidecar와 평가 복원 경로에 기록했다. 0 이하 값은 network 초기화에서 거부한다.
- A network·학습·checkpoint parameter shape는 변경하지 않았다. zero-init residual이므로 B 시작 action과 GO의 gate=0 action은 여전히 frozen A와 정확히 같다.
- Python compile, train/eval shell syntax와 diff check를 통과했다. 장시간 학습이나 GPU rollout은 시작하지 않았다.


## 2026-08-28 · frozen A stop residual과 cosine resume

- frozen A latent와 6-D phase command를 받는 zero-init `stop_residual`을 추가했다. action은 `a_base + gate*delta`이며 B에서는 A 전체와 input RMS를 동결하고 residual만 학습한다.
- BRAKE 1→0, RESUME 0→1 cosine scale과 transition Huber speed tracking을 추가했다. HOLD 중 root/box가 모두 0.2 m/s 아래였던 경우에만 이후 resume 성공을 인정한다.
- `carryWith` B 전용 residual train 실행기와 sidecar 복원 eval을 추가했다. 이후 floor-pickup A/B 회귀를 같은 checkpoint로 평가할 수 있다.
- GPU 6 smoke에서 residual 4/4 parameter만 gradient를 받았고 strict checkpoint load가 통과했다. 48-episode latch 평가에서 brake/resume 각 6.2 step, 실제 정지 미달 시 `resumed=0`을 확인했다. Python compile과 shell syntax/diff check도 통과했다.

## 2026-08-28 · pickup 유지 원인 분리 GUI

- `scripts/mastop/view_pickup_diag.sh <1|2>`로 A checkpoint/B layout/stop-off와
  B checkpoint/A layout을 같은 GPU 6·noVNC 6100 계약에서 비교하게 했다.
- 범용 GUI의 Vulkan UUID guard를 그대로 사용하고, 정식 carry 평가와 같게
  `--test --eval --eval_task carry`를 적용한다.

## 2026-08-27 · mastop 평가 metric direct capture 및 Gate A 재검증

- MASteer의 검증된 `begin_eval_metrics` / `capture_eval_metrics` / `end_eval_metrics` 경로를 mastop env와 transformer player에 이식했다. 평가 성공률에 실제 포함된 done row만 같은 step에서 저장하고, 평가 중 일반 reset 기반 dump는 막아 마지막 batch 누락과 미완료 episode 혼입을 함께 제거했다.
- `stage2_ma_stop_carry_eval.sh`는 이제 `num_envs × 2 agents × 3 repeats`와 metric 행 수가 다르면 결과를 거부한다. 8-env smoke는 정확히 48행, 512-env Gate A는 정확히 `(3072, 60)`을 생성했다.
- `pickupdiag_live_15k_s0` 6k의 Gate A 재평가는 strict pickup `0.9137`, finish `0.9121`, `test_1+2` 목표 도달률 `0.8970`이다. 기존 strict `0.9160`, 목표 도달률 `0.8999` 대비 각각 -0.23%p, -0.29%p로 carry 회귀 gate를 통과했다.

## 2026-08-27 · pickupdiag live 6k 기반 gradual carry stop

- `HumanoidMAStopCarry`의 즉시 `M=0` 전환을 `MSTOP_BRAKE_T`(기본 0.8초) cosine ramp로 바꿨다. BRAKE에서는 낮아지는 steering 명령과 박스 유지 보상만 사용하고, 명령이 0이 된 HOLD에서만 root/box 정지·creeping 보상을 적용한다.
- BRAKE 시간을 hold latch에 포함하고, 재출발은 non-yield 사람과 박스가 모두 교차점을 지난 뒤 footprint clearance가 확보될 때만 허용한다. Dynamic yield는 사람-사람·사람-박스·박스-사람·박스-박스의 3초 미래 clearance를 사용한다.
- train/eval 기본을 pickupdiag 조건(`MA_TOKEN=live`, `MS_MRAND=0`, `MS_VEL_W=1`, `MS_CLIP=0`)으로 맞췄고, Gate A만 `MA_SEP=9`, B-D는 `MA_SEP=0`을 쓴다. 실행기 시작 checkpoint는 `pickupdiag_live_15k_s0` 6k다.
- Python compile과 train/eval/runner bash syntax를 통과했다. GPU 6의 8-env Stage B 무학습 smoke는 rc=0, obs 340-D, metric `(36, 60)`, 평균 brake 20.8 step, `heldStop=1.0`, `resumed=0.8125`였다. HOLD root/box 속도는 1.388/1.392 m/s로, gradual stop fine-tuning이 필요함을 확인했다.


## 2026-08-26 · mastop noVNC 뷰어의 물리 GPU 고정

- `scripts/mastop/view.sh`가 CUDA만 마스킹하던 경로에 Vulkan device-select를
  추가했다. 이 서버에서는 PCI 주소 selector가 NVIDIA 8장을 그대로 노출하므로,
  실제 장치 1개만 노출하는 숫자 selector `DRI_PRIME=6!|7!`를 사용한다.
- 프로세스 내부에서는 마스킹된 카드를 `cuda:0` / `graphics_device_id=0`으로
  일관되게 사용하며, 6·7 이외의 물리 GPU는 스크립트 시작 전에 거부한다.
- Vulkan 열거 진단에서 `6!`는 UUID `8d9172bc-...`/PCI `e6:00.0`, `7!`는
  UUID `3e1773be-...`/PCI `e7:00.0` 한 장만 노출하는 것을 확인했다.
- 공통 `scripts/vulkan_gpu_guard.py`가 장치 수와 UUID를 실행 직전에 다시 검증하며,
  불일치하면 Xvfb와 Isaac Gym을 시작하기 전에 실패한다.
- 같은 fail-closed 가드를 `steer`, `ma`, `masteer`, `mastop`의 네
  `view.sh`, 관련 `record.sh`, 범용 `tokenhsi_gui.sh`에 연결해 과거 패턴을
  복사해도 물리 GPU 0 사용을 차단한다.

## 2026-08-25 · MASteer 기반 carry stop A/B/C/D

- object-free `HumanoidMAStopSteer`는 보존하고, 검증된 `HumanoidMASteerCarry`를 동기화한 뒤 이를 상속하는 `HumanoidMAStopCarry`를 추가했다. strict pickup 10-frame latch와 anti-kicking height gate를 그대로 유지한다.
- 관측·네트워크 크기 340-D는 바꾸지 않고 기존 steering window의 `M=0`으로 post-pickup stop을 명령한다. A free, B solo hold/resume, C fixed yield, D ETA 기반 dynamic yield를 누적 혼합한다.
- GPU 6·7 DDP train, full strict eval, 단계별 gate, detached A→B→C→D chain을 추가했다. Python compile·bash syntax, A/B rollout 58-column metric, 2-rank resume/backward/checkpoint smoke를 통과했다.
- 학습 전 B smoke는 pickup 1.0·stop/resume 1.0이지만 root/box stop speed가 약 0.90 m/s여서, gate는 실제 stop speed 0.6 m/s 이하까지 요구한다.


## 2026-08-25 · mastop 충돌 케이스 noVNC 뷰어

- `scripts/mastop/view.sh`를 추가해 mastop 체크포인트·config를 스냅샷으로 읽고
  Xvfb, x11vnc, websockify를 한 번에 띄우도록 했다. 기본 시나리오는
  `MSTOP_SCEN=mixed MSTOP_MIX=0,0,0,1`인 conflict 전용이다.
- `HumanoidMAStopSteer`에 `MS_CAM=top` 고정 overview 카메라를 추가했다.
  설정하지 않으면 기존 follow 카메라 동작은 그대로다.
- `bash -n`, Python compile 통과. GPU 6·임시 포트에서 Xvfb/x11vnc/noVNC,
  GPU PhysX, A=2 conflict rollout과 제한 종료 후 보조 프로세스 정리까지 확인했다.

## 2026-08-24 · 단일 Stage2 mixed stop/wait/resume curriculum

### 구현

- `HumanoidMAStopSteer`에 `MSTOP_SCEN=mixed`를 추가했다. 하나의 정책과 하나의
  Stage2 run 안에서 episode마다 `free / solo / near / conflict`를 기본
  `0.35 / 0.20 / 0.20 / 0.25` 비율로 다시 뽑는다.
- 교차 episode는 전역 회전, 60~120도 교차각, ETA 차이, yield agent를 무작위화한다.
  near는 충분한 ETA 간격을 주고 conflict는 거의 동시 도착시켜 한 geometry 암기를 막는다.
- oracle의 미래 위치는 고정 nominal speed가 아니라 실제 path-forward velocity EMA로
  예측한다. 위험이 생기면 뒤에 도착하는 agent의 trajectory window를 `M=0`으로 latch하고,
  상대가 conflict zone을 통과한 뒤 release한다. solo도 정지 위치·hold 시간을 무작위화한다.
- 기존 Stage1의 354-D 관측·actor 구조를 그대로 유지했다. 상위 oracle만 trajectory
  command를 바꾸며 low-level policy는 stop, hold, resume를 물리적으로 실행한다.
- 정상 구간 속도 보상 비중을 0.65로 두고 HOLD 중 creeping penalty를 추가해 항상
  느리게 가는 해법을 억제했다.
- scenario, collision, stop/go speed, both-stop, resumed, resume latency, predicted
  minimum distance/time을 episode 시점에 직접 기록한다. 평가용 per-agent
  success/precision도 path completion/lateral error로 연결했다.
- mixed A=2·2048 env cfg와 train/eval 스크립트를 추가했다. 학습은 물리 GPU 6·7을
  torchrun 2-rank DDP로 함께 사용하며 env와 minibatch를 rank별로 나눈다. 두 rank의
  curriculum seed도 분리했다. 평가는 물리 GPU
  6 또는 7 중 하나만 허용한다. Stage1 checkpoint epoch에 Stage2 iteration을
  더해 한 번의 full fine-tuning으로 실행한다.
- 공통 학습 루프의 max-epoch 종료를 rank 0 전용 블록 밖으로 옮겼다. checkpoint는
  rank 0만 저장하되 모든 DDP rank가 같은 epoch에 return하므로 종료 collective hang이 없다.

### 검증

- `py_compile`, `bash -n` 통과.
- 물리 GPU 6·7 동시 DDP, A=2·총 8 env(4 env/rank), 1 iteration 종료 smoke:
  Stage1 epoch 40000 로드, 양 rank의 354-D observation, rollout, backward,
  checkpoint 저장 및 torchrun 자체 exit code 0까지 통과.
- GPU 6, A=2·16 env evaluation: 3회 평가 성공률 0.96875 / 0.96875 / 1.0.
  36 episode metric 생성. 미학습에 가까운 smoke 정책이라 성능 결론에는 쓰지 않지만,
  free/near stopRate=0, solo/conflict stopRate=1·resumed=1, collision=0으로
  scenario 분기와 stop/release/metric 경로가 모두 실행됨을 확인했다.

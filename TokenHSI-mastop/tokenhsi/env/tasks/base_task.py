...
class BaseTask():
    ...
    def __init__(self, cfg, enable_camera_sensors=False):
        ...
        # 그게 유일한 headless 녹화 경로다.
        import os as _os
        if _os.environ.get("MA_VIDEO"):
            enable_camera_sensors = True
        if enable_camera_sensors == False and self.headless == True:
            self.graphics_device_id = -1

        self.num_envs = cfg["env"]["numEnvs"]
        # Humanoid sets num_agents before calling us. Buffers split into two
        # kinds and the split is the whole multi-agent design:
        #
        #   per-agent (_rows)   obs / states / rew    one row per (env, agent)
        #   per-env   (num_envs) reset / progress     the scene is shared, so
        #                                             the episode is shared too
        #
        # Agents cannot restart independently: the loser would respawn inside a
        # scene the other agent is still working in, and every team metric
        # (makespan, deadlock, success_both) is defined over one shared episode.
        # With one agent the two are the same length, so nothing changes.
        self._rows = self.num_envs * getattr(self, "num_agents", 1)
        ...
        self._physics_trace_path = os.environ.get("MSTOP_PHYSICS_PATH")
        self._physics_trace_steps = int(os.environ.get("MSTOP_PHYSICS_STEPS", "8"))
        self._physics_trace_step = 0
        if self._physics_trace_path:
            parent = os.path.dirname(self._physics_trace_path)
            if parent:
                os.makedirs(parent, exist_ok=True)
            open(self._physics_trace_path, "w").close()
        ...
        self.obs_buf = torch.zeros(
            (self._rows, self.num_obs), device=self.device, dtype=torch.float)
        self.states_buf = torch.zeros(
            (self._rows, self.num_states), device=self.device, dtype=torch.float)
        self.rew_buf = torch.zeros(
            self._rows, device=self.device, dtype=torch.float)
        ...
        self.save_video = getattr(cfg["args"], "record", False)
    ...
    def _physics_step(self):
        trace = (self._physics_trace_path
                 and self._physics_trace_step < self._physics_trace_steps)
        sim_times = [self.gym.get_sim_time(self.sim)] if trace else None
        for i in range(self.control_freq_inv):
            self.render()
            self.gym.simulate(self.sim)
            if trace:
                sim_times.append(self.gym.get_sim_time(self.sim))
        if trace:
            record = {
                "step": self._physics_trace_step,
                "control_freq_inv": self.control_freq_inv,
                "sim_times": sim_times,
            }
            with open(self._physics_trace_path, "a") as f:
                f.write(json.dumps(record) + "\n")
            self._physics_trace_step += 1
        return

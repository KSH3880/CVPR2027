...
class VecTask():
    def __init__(self, task, rl_device, clip_observations=5.0, clip_actions=1.0):
        ...
        self.num_agents = getattr(task, 'num_agents', 1)  # used for multi-agent environments
...
class VecTaskPython(VecTask):
    ...
    def step(self, actions):
        ...
        # reset_buf is per-env; obs and rew are per-(env, agent). Hand rl_games
        # one done per row so every agent of an env ends together.
        dones = self.task.reset_buf
        if self.num_agents > 1:
            dones = dones.repeat_interleave(self.num_agents)

        return torch.clamp(self.task.obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device), self.task.rew_buf.to(self.rl_device), dones.to(self.rl_device), self.task.extras

    def reset(self):
        rows = self.task.num_envs * self.num_agents
        actions = 0.01 * (1 - 2 * torch.rand([rows, self.task.num_actions], dtype=torch.float32, device=self.rl_device))

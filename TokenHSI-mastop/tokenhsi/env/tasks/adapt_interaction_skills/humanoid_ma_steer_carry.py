...
class HumanoidMASteerCarry(HumanoidMACarry):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        # 실제 pickup 평가: 손 근접 + 초기 지지면에서 lift + 연속 유지.
        self.pick_hand_dist = _f("MS_PICK_HAND", 0.35)
        self.pick_lift = _f("MS_PICK_LIFT", 0.10)
        self.pick_frames = int(_f("MS_PICK_FRAMES", 10))
        assert self.pick_hand_dist > 0 and self.pick_lift > 0
        assert self.pick_frames >= 1
        self._steer_tick = 0
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # pickup은 proximity proxy가 아니다. 리셋 직후 박스 높이를 직접 저장하고,
        # 손 근접 + lift가 연속 pick_frames 동안 유지돼야 성공으로 latch한다.
        self._ep_picked = torch.zeros(R, device=self.device, dtype=torch.bool)
        self._ep_held_steps = torch.zeros(R, device=self.device, dtype=torch.long)
        self._pick_z0 = torch.zeros(R, device=self.device)
        self._pick_streak = torch.zeros(R, device=self.device, dtype=torch.long)
        self._ep_pick_max_streak = torch.zeros(R, device=self.device, dtype=torch.long)
        self._ep_pick_max_lift = torch.zeros(R, device=self.device)
        self._ep_close_any = torch.zeros(R, device=self.device, dtype=torch.bool)
        self._ep_close_steps = torch.zeros(R, device=self.device, dtype=torch.long)
    ...
    def _post_object_reset(self, env_ids):
        ...
        **_reset_envs 뒤에 하면 안 된다** -- 그 시점엔 부모가 이미
        ...
        if len(env_ids) > 0:
            ...
            # 물체 reset·layout·sim refresh가 모두 끝난 실제 시작 높이를 직접 기록한다.
            self._pick_z0[rows] = self.humanoid_rows(self._box_states)[rows, 2]
            self._pick_streak[rows] = 0
    ...
    def _compute_reward(self, actions):
        ...
        box_vel_r = vel_term(box_vel3[:, 0:2], u_box, pin_carry, v_box)
        # 원본 TokenHSI carry와 같은 anti-kicking gate. 박스가 지면에서
        # 0.2 m 이상 들리기 전에는 운반 속도로 보상받지 못한다. 원본은 이
        # 마스크 뒤에 dist_mask를 적용하므로 목표 0.5 m 안의 pin은 그대로 둔다.
        low_box = box_pos[:, 2] <= (self._box_lib._box_size[:, 2] / 2 + 0.2)
        box_vel_r = torch.where(low_box & ~pin_carry, torch.zeros_like(box_vel_r), box_vel_r)
        carry_r = _vw * box_vel_r + 0.2 * pos_near
    ...
    def update_metrics(self):
        ...
        close = torch.norm(hands - box, dim=-1) < self.pick_hand_dist
        lift = box[:, 2] - self._pick_z0
        lifted_held = close & (lift >= self.pick_lift)

        self._ep_close_any |= close
        self._ep_close_steps += close.long()
        self._ep_pick_max_lift = torch.maximum(self._ep_pick_max_lift, lift)

        self._pick_streak = torch.where(
            lifted_held, self._pick_streak + 1, torch.zeros_like(self._pick_streak))
        self._ep_pick_max_streak = torch.maximum(self._ep_pick_max_streak, self._pick_streak)
        self._ep_held_steps += lifted_held.long()
        self._ep_picked |= self._pick_streak >= self.pick_frames
    ...
    def _metric_extra_cols(self, rows):
        ...
        cols.append(self._ep_picked[rows].float())                # 40 실제 pickup 여부
        cols.append(self._ep_held_steps[rows].float())            # 41 lifted+held 프레임
        cols.append(self._ep_close_any[rows].float())             # 42 옛 proximity-only 여부
        cols.append(self._ep_close_steps[rows].float())           # 43 proximity 프레임
        cols.append(self._pick_z0[rows])                           # 44 reset 직후 box z
        cols.append(self._ep_pick_max_lift[rows])                  # 45 최대 lift
        cols.append(self._ep_pick_max_streak[rows].float())        # 46 최대 연속 lifted+held
        return cols
    ...
    def _metric_reset_extra(self, rows):
        ...
        self._ep_picked[rows] = False
        self._ep_held_steps[rows] = 0
        self._pick_z0[rows] = 0.0
        self._pick_streak[rows] = 0
        self._ep_pick_max_streak[rows] = 0
        self._ep_pick_max_lift[rows] = 0.0
        self._ep_close_any[rows] = False
        self._ep_close_steps[rows] = 0
        return

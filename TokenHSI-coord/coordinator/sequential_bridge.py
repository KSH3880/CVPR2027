...
def carry_rows(phase, rehearsal):
    """Stack phases: place/verify=0/1, retreat=2, A2/verify=3/4, done=5."""
    return torch.stack(((phase == 0) | (phase == 1),
...
def plan_validity(path, speed, state, active):
    ...
    # Zero-length legs are legitimate at pickup/placement; no direction to turn.
...
def select_stack_plan(output, state, active):
    """Safety-first selection with inactive agents/boxes stationary in prediction.

    Inactive rows remain governed by the native waiting/retreat behavior in the
    simulator. Their future motion is approximated as stationary here, rather
    than scoring the unexecuted Carry paths emitted by the learned model.
    """
    path, speed = output["path_world"], output["speed"]
    valid = plan_validity(path, speed, state, active)
    # Sanitize invalid candidates before time interpolation, but never accept them.
    ...
    cost = torch.where(valid, cost + (~safe).float() * 1e6, torch.full_like(cost, float("inf")))
...
def resample_plan(path, speed):
    """Interpolate XY and speed on the SAME path-distance axis (0.1m/320).

    Speed is a scalar attached to a spatial waypoint, not its own polyline.
    """

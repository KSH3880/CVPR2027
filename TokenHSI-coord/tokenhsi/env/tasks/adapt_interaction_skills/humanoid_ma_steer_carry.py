        ...
        for i, env_ptr in enumerate(self.envs):
            for a in range(A):
                ...
                if draw_speed:
                    ...
                    # C2는 속도가 연속적으로 변하므로 같은 값의 run만 그리면 거의 모든
                    # segment가 누락된다. 각 segment의 양 끝 평균 속도로 띠 전체를 한 번에
                    # 채색한다.
                    ms = mscale_np[r][:max(end, 2)][::2]
                    v = band(g, 0.04 + 0.01 * a, gt_w)
                    segment_scale = 0.5 * (ms[:-1] + ms[1:])
                    repeats = len(v) // max(len(segment_scale), 1)
                    segment_scale = np.tile(segment_scale, repeats)
                    base = np.asarray(GT_COL[a % len(GT_COL)], dtype=np.float32)
                    shade = base[None] * (0.28 + 0.72 * segment_scale[:, None])
                    self.gym.add_lines(self.viewer, env_ptr, len(v), v, shade.astype(np.float32))
                else:
                    v = band(g, 0.04 + 0.01 * a, gt_w)

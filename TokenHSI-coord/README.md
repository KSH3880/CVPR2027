...
`TokenHSI-masteer`의 검증된 ms18 action executor 앞에 state-only joint trajectory
coordinator를 붙이는 독립 연구 저장소다. coordinator는 두 agent/box/goal의 실제
state에서 이름 없는 `K=4` joint future를 만들고, 안전성과 소요시간으로 하나를 골라
기존 6점/12-D steer token으로 변환한다. ms18 PTH는 수정하거나 재저장하지 않는다.

구조와 실행법은 [`coordinator/README.md`](coordinator/README.md)에 있다. 아래는
기반으로 사용한 upstream TokenHSI 설명이다.

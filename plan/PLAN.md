...
- 새 잡의 최소 메모리 여유 26,000 MiB가 없으면 제외한다.
- 이 저장소는 GPU 0 하나만 사용하고, 큐 전체에서 동시에 한 작업만 실행한다.

### 선택 순서

1. GPU 0만 후보로 본다.
2. GPU 0에 작업이 없고 최소 메모리와 5분 정착 조건을 만족할 때만 새 작업을 시작한다.
3. GPU 0이 사용 중이거나 메모리가 부족하면 기다린다. 두 번째 작업을 얹지 않는다.
4. polling 한 번에는 학습 또는 평가 하나만 시작한다.
5. 자동 최종 평가가 있는 트랙은 미평가 체크포인트를 새 학습보다 먼저 배치한다.

    ...
    class Network(AMPTransformerMultiTaskBuilder.UnifiedNetworkClass):
        ...
        def __init__(self, params, **kwargs):
            ...
            if self.has_extra:
                ...
                enc = self.task_encoder[self.extra_id]
                # **마지막 활성함수를 반드시 걷어낸다.** _build_mlp 는 units 의 모든 항목
                # 뒤에 활성을 붙이는데, 여기서는 units 마지막이 곧 토큰 차원이라 출력 뒤에
                # ReLU 가 남는다. 그 상태로 마지막 Linear 를 0 으로 두면 활성 전 값이
                # 정확히 0 이고 PyTorch 의 ReLU'(0)=0 이라 **backward 가 0 을 곱해
                # 토크나이저 전체가 영원히 안 배운다.**
                #
                # 실측(2026-08-20): t1_live_s0 · t6_nf_c0 · g1_live_s1 전부 teammate
                # 토크나이저가 초기값 그대로였다 (max 가 1/sqrt(fan_in) 과 정확히 일치).
                # grad 합도 정확히 0. 즉 **live 와 zero 가 같은 네트워크였고**,
                # "토큰 효과 없음 Δ=+0.004" 은 토큰이 두 조건 모두에서 0 이었던 것이다.
                # 입력축 결론 전체를 다시 재야 한다.

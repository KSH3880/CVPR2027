#!/bin/bash
# Exact A=1 parity viewer for the single-agent f22 carry+steer checkpoint.
# This path intentionally does not use HumanoidMASteerCarry or the modified
# masteer adapt builder.
#
#   PORT=6101 MA_GPU=7 bash scripts/f22/view_a1.sh /abs/path/Humanoid_00016000.pth
set -e
...
#   <project>/TokenHSI-masteer/scripts/f22/view_a1.sh
#   <project>/scripts/f22/view_a1.sh
...
CKPT=${1:?usage: view_a1.sh <f22-checkpoint> [num-envs]}
ENVS=${2:-1}
PORT=${PORT:-6101}
MA_GPU=${MA_GPU:-0}
...
python ./tokenhsi/run.py --task HumanoidF22SteerCarry \
    ...
    --cfg_env tokenhsi/data/cfg/adapt_interaction_skills/amp_humanoid_f22_steer_carry_cp32.yaml \

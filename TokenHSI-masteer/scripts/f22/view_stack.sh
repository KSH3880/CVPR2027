...
PORT=${PORT:-6101}
MA_GPU=${MA_GPU:-7}
MA_GRAPHICS_GPU=${MA_GRAPHICS_GPU:-$MA_GPU}
...
case "$MA_GPU" in
    6|7) ;;
    *)
        echo "view_stack.sh is restricted to physical GPU 6 or 7; got MA_GPU=$MA_GPU"
        exit 1
        ;;
esac
case "$MA_GRAPHICS_GPU" in
    0|1|2|3|4|5|6|7) ;;
    *)
        echo "MA_GRAPHICS_GPU must be a Vulkan device index in 0..7; got $MA_GRAPHICS_GPU"
        exit 1
        ;;
esac

# Expose only the selected physical CUDA device before any Python process
# imports torch.  PyTorch/PhysX consequently address it as logical cuda:0;
# graphics remains a physical Vulkan ordinal and is passed separately below.
...
cd "$REPO"
export DISPLAY="$DISP"
export TOKENHSI_GRAPHICS_DEVICE_ID="$MA_GRAPHICS_GPU"
...
echo "CUDA_VISIBLE_DEVICES=$MA_GPU; CUDA/PhysX logical cuda:0; RL logical cuda:0; Vulkan device index $MA_GRAPHICS_GPU"
...
python ./tokenhsi/run.py --task HumanoidMAF22StackCarry \
    --sim_device cuda:0 --rl_device cuda:0 --graphics_device_id "$MA_GRAPHICS_GPU" \
    --physx --pipeline gpu \

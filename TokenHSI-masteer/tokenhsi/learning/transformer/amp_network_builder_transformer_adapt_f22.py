...
class AMPTransformerMultiTaskAdaptBuilder(AMPBuilder):
    ...
    class Network(AMPTransformerMultiTaskBuilder.UnifiedNetworkClass):
        ...
        def __init__(self, params, **kwargs):
            ...
            import os as _os
            _ab = _os.environ.get("STEER_ABLATE", "").strip()
            self.ablate_token_ids = [int(x) for x in _ab.split(",") if x] if _ab \
                else params.get("ablate_token_ids", [])
            self.extra_tokenizer_zero_init = bool(int(_os.environ.get(
                "STEER_TOKENIZER_ZERO", "0"))) or params.get("extra_tokenizer_zero_init", False)
            ...
            if self.has_extra:
                ...
                for m in self.task_encoder[self.extra_id].modules(): # zero init
                    if isinstance(m, nn.Linear):
                        if self.extra_tokenizer_zero_init:
        ...
        def _eval_Transformer(self, obs, not_normalized_obs):
            ...
            if self.ablate_token_ids:
                # padding a token out of attention is the only exact "absent"
                # test; a zeroed token still shifts the softmax
                src_key_padding_mask[:, self.ablate_token_ids] = True
